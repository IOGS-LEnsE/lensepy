...
class CameraIDS:
    ...
    def stop_acquisition(self):
        ...
        if self.data_stream is not None:
            ...
            try:
                self.data_stream.StopAcquisition()
                self.data_stream.Flush(ids_peak.DataStreamFlushMode_DiscardAll)  # 🔴 important
            except Exception:
                pass
    ...
    def get_image(self, fast_mode: bool = True) -> np.ndarray:
        ...
        if self.camera_connected and self.camera_acquiring:
            ...
            buffer = self.data_stream.WaitForFinishedBuffer(100000)
            # convert to RGB
            ...
            if self.color_mode == 'Mono12g24IDS':  # NOT YET IMPLEMENTED FOR CONVERSION ! See __init__.py
                ...
                picture = raw_image.get_numpy_3D().copy().squeeze()
            else:
...
if __name__ == "__main__":
    ...
    def capture_image(my_cam):
        ...
        global displayed
        print('Capture')
        my_cam.start_acquisition()  # allocate buffer to store raw data from the camera
